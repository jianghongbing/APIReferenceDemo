...
//    self.textView = [self addCustomTextViewOne:[LayoutManagerTwo new] textContainer:[CustomTextContainer new] height:200];
...
- (IBAction)startDraw:(id)sender {
    LayoutManagerTwo *layoutManager = (LayoutManagerTwo *)self.textView.layoutManager;
    CGPoint origin = layoutManager.textDrawOrigin;
    origin.x += 10;
    origin.y += 10;
    layoutManager.textDrawOrigin = origin;
    

    
}
- (IBAction)drawUnderLine:(id)sender {
    LayoutManagerTwo *layoutManager = (LayoutManagerTwo *)self.textView.layoutManager;
    NSTextStorage *textStorage = self.textView.textStorage;
//    [textStorage addAttributes:@{NSBackgroundColorAttributeName: [UIColor redColor], NSForegroundColorAttributeName: [UIColor greenColor], NSUnderlineColorAttributeName: [UIColor blackColor], NSUnderlineStyleAttributeName: @(2)} range:NSMakeRange(50, 100)];
    [textStorage addAttributes:@{NSUnderlineColorAttributeName: [UIColor orangeColor], NSUnderlineStyleAttributeName: @(2)} range:NSMakeRange(50, 100)];

    [layoutManager invalidateDisplayForGlyphRange:NSMakeRange(50, 100)];
}
- (IBAction)drawStrikethrough:(id)sender {
    LayoutManagerTwo *layoutManager = (LayoutManagerTwo *)self.textView.layoutManager;
    NSTextStorage *textStorage = self.textView.textStorage;
    [textStorage addAttributes:@{NSStrikethroughColorAttributeName: [UIColor redColor], NSStrikethroughStyleAttributeName: @(1)} range:NSMakeRange(100, 120)];
    [layoutManager invalidateDisplayForGlyphRange:NSMakeRange(100, 120)];
    

//    [layoutManager invalidateDisplayForGlyphRange:NSMakeRange(0, 100)];
}
@end
